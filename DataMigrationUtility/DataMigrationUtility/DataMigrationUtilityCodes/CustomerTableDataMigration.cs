...
{
    class CustomerTableDataMigration
    {
        ...
        public static void GetMasterClientIdForCustomer()
        {
            object data;
            string datas = "";
        ...
        public static void CustomerTableMigration()
        {
            int index = 0;

            try
            {
                GetMasterClientIdForCustomer();
                using (MySqlConnection connection = new MySqlConnection(cs1))
                {
                    connection.Open();
                    using (MySqlCommand command = new MySqlCommand("SELECT DISTINCT AccountID from customer ORDER BY AccountID ASC", connection))
                    {
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                {
                                    connection1.Open();
                                    using (MySqlCommand command1 = new MySqlCommand("SELECT Name,CompanyName,Address1,City,Country,Zip,VATNumber,WorkPhone,MobilePhone,Email,Status,Region,IsRevAlow FROM customer Where AccountID =" + ClientId, connection1))
                                    {
                                        using (MySqlDataReader reader1 = command1.ExecuteReader())
                                        {
                                            while (reader1.Read())
                                            {
                                                ...
                                                var q = "INSERT INTO customer(ContactName,CompanyName,CompanyAddress,CompanyCity,CompanyCountry,CompanyPostcode,VATNumber,CompanyPhone,ContactPhone,ContactEmail,Status,CompanyCounty,IsRevAllow) Values(" + datas + "); ";
