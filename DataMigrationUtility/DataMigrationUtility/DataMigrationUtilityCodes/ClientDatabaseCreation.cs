...
namespace DataMigrationUtility.DataMigrationUtilityCodes
{
    class ClientDatabaseCreation
    {
        ...
        public static void CreateClientFolderForAWS(object clientID)
        {
            ...
            object VatNumber;
            object AccountIdForImageUpload;
            ...
            try
            {
                ...
                {
                    connection1.Open();
                    using (MySqlCommand command1 = new MySqlCommand("SELECT AccountID FROM account ORDER BY AccountID ASC", connection1))
                    {
                        using (MySqlDataReader reader1 = command1.ExecuteReader())
                        {
                            while (reader1.Read())
                            {
                                AccountIdForImageUpload = reader1.GetValue(0);
                                using (MySqlConnection connection = new MySqlConnection(cs1))
                                {
                                    connection.Open();
